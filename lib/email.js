...
export function quoteEmailHtml({ name, plan, amountEur, paymentMode, paymentLink }) {
  ...
  return wrap(`
    ...
    <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:12px;padding:20px 24px;margin:20px 0;">
      <span style="font-family:ui-monospace,Menlo,monospace;font-size:11px;color:#6b7280;">${paymentMode === 'balance' ? 'balance due' : paymentMode === 'deposit' ? '50% deposit' : 'total'}</span>
      <div style="color:#111827;font-size:32px;font-weight:700;">${amount}</div>
    </div>
    ${modeLine}
    ${button(paymentLink, paymentMode === 'balance' ? 'Pay balance — go live' : 'Pay securely & book your week')}
