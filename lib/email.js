...
// Signature block appended to every email — photo, name, role, links.
// Table layout + inline styles for email-client compatibility.
const SIGNATURE = `
  <table cellpadding="0" cellspacing="0" role="presentation" style="width:100%;margin-top:28px;border-top:1px solid #e5e7eb;">
    <tr>
      <td style="padding-top:20px;vertical-align:middle;width:52px;">
        <img
          src="https://www.web-frame.eu/images/bruno.webp"
          width="48" height="48" alt="Bruno Čukić"
          style="display:block;border-radius:50%;width:48px;height:48px;object-fit:cover;"
        />
      </td>
      <td style="padding-top:20px;padding-left:14px;vertical-align:middle;">
        <div style="color:#111827;font-size:14px;font-weight:700;line-height:1.3;">Bruno Čukić</div>
        <div style="color:#6b7280;font-size:12px;line-height:1.5;">Founder &amp; Developer · Webframe</div>
        <div style="font-size:12px;line-height:1.5;">
          <a href="mailto:hello@web-frame.eu" style="color:#4b2bff;text-decoration:none;">hello@web-frame.eu</a>
          <span style="color:#d1d5db;">&nbsp;·&nbsp;</span>
          <a href="https://www.web-frame.eu" style="color:#4b2bff;text-decoration:none;">web-frame.eu</a>
        </div>
      </td>
      <td style="padding-top:20px;vertical-align:middle;text-align:right;">
        <span style="font-family:ui-monospace,Menlo,monospace;font-size:10px;color:#9ca3af;">// live in 7 days</span>
      </td>
    </tr>
  </table>`
  ...
  const modeLine =
    paymentMode === 'balance'
      ...
      : paymentMode === 'monthly'
        ...
        : `<p style="color:#374151;font-size:15px;line-height:1.6;">One payment, everything included — your build week is booked the moment it lands.</p>`
  return wrap(`
    ...
    <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:12px;padding:20px 24px;margin:20px 0;">
      <span style="font-family:ui-monospace,Menlo,monospace;font-size:11px;color:#6b7280;">${paymentMode === 'balance' ? 'balance due' : paymentMode === 'monthly' ? 'per month' : 'total'}</span>
...
export function confirmationEmailHtml({ name }) {
  const first = name ? name.split(' ')[0] : 'there'
  return wrap(`
    <h1 style="color:#111827;font-size:22px;margin:0 0 16px;">Got your brief — you'll hear from me soon</h1>
    <p style="color:#374151;font-size:15px;line-height:1.6;">Hi ${first},</p>
    <p style="color:#374151;font-size:15px;line-height:1.6;">
      Thanks for reaching out about your website. I read every inquiry
      personally and you'll get a reply <strong>within 24 hours</strong> —
      usually with a proposed time for a free 15-minute call and a fixed
      quote shortly after.
    </p>
    <p style="color:#374151;font-size:15px;line-height:1.6;">
      In the meantime, feel free to reply to this email with anything that
      helps: links to sites you like, your current site, or questions.
    </p>
  `)
}
...
export function loginCodeEmailHtml({ code }) {
  return wrap(`
    <h1 style="color:#111827;font-size:22px;margin:0 0 16px;">Your portal login code</h1>
    <p style="color:#374151;font-size:15px;line-height:1.6;">
      Enter this code to open your client portal. It expires in 10 minutes.
    </p>
    <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:12px;padding:20px 24px;margin:20px 0;text-align:center;">
      <span style="font-family:ui-monospace,Menlo,monospace;font-size:32px;font-weight:700;letter-spacing:8px;color:#111827;">${code}</span>
    </div>
    <p style="color:#6b7280;font-size:13px;line-height:1.6;">
      Didn&rsquo;t request this? You can safely ignore this email — nobody can
      access your portal without the code.
    </p>
  `)
}
...
export function ticketReplyEmailHtml({ name, subject, reply, portalUrl }) {
  const first = name ? name.split(' ')[0] : 'there'
  return wrap(`
    <h1 style="color:#111827;font-size:22px;margin:0 0 16px;">Re: ${escapeHtml(subject)}</h1>
    <p style="color:#374151;font-size:15px;line-height:1.6;">Hi ${first},</p>
    <p style="color:#374151;font-size:15px;line-height:1.6;">
      You have a new reply on your support ticket:
    </p>
    <div style="background:#f9fafb;border-left:3px solid #4b2bff;border-radius:0 12px 12px 0;padding:16px 20px;margin:20px 0;">
      <p style="color:#374151;font-size:14px;line-height:1.7;margin:0;white-space:pre-wrap;">${escapeHtml(reply)}</p>
    </div>
    ${portalUrl ? button(`${portalUrl}/support`, 'View & reply in your portal') : ''}
    <p style="color:#6b7280;font-size:13px;line-height:1.6;margin-top:20px;">
